...
- [ ] Daily accrual job: per fund, `invested_value × 0.001 / 365`, summed into a
      ...
      - **Greater-of guard (see `ONE_METER_FEE_DECISION.md`):** if the fund is on a
        paid plan whose subscription is currently the LARGER fee, accrue NOTHING —
        the subscription already covers it. Accrue AUM only where AUM exceeds the
        sub (post-crossover funds + post-handoff inherited funds with no sub).
        Accruing unconditionally would re-create the sub+AUM double-charge.
