...
export function buildParentHandoffRecurringEmail(
  ...
  const intro = [
    ...
    ``,
    `If you want to keep showing up, you can set up the same ${amount} ${cadence} contribution as a recurring gift through ${childFirstName}'s gift link. Same cadence, same amount, same fund. The only difference is it comes from you as a gifter now, not as the custodian. ${childFirstName} sees who it's from. Cancel any time.`,
    ``,
