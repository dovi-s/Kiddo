...
async function tick(log: LogFn): Promise<void> {
  ...
  for (const row of rows) {
    ...
    // Respect per-category opt-out. Missing key in the parent's
    // preferences map = opted in (the default).
    if (!isCategoryEnabled(row.parentEmailPreferences, "birthday")) continue;
