...
export async function registerRoutes(
  ...
  app.get('/api/kid-view/:token/content', async (req, res) => {
    try {
      ...
      //
      // ALSO filtered: legacy 'parent_investment_start' entries and any
      // entry whose content matches the auto-invest transactional log
      // pattern ("Dovi added $50 into ADBE to Emma's fund."). These were
      ...
      const entries = entriesAll.filter((e: any) => {
        ...
        if (content && AUTO_INVEST_MEMORY_RE.test(content)) return false;
        return true;
      });
      ...
      res.json({
        ...
        gifts: fundGifts.slice(0, ageInfo.phase === "teen" ? 12 : 5).map((gift) => {
          ...
          const isAutoInvestBoilerplate = /^auto-invest contribution to /i.test(rawMessage);
          return {
            id: gift.id,
            senderName: gift.senderName,
            amount: gift.amount,
            message: isAutoInvestBoilerplate ? null : gift.message,
      ...
      try {
        ...
        await sendEmail({
          ...
          text: [
            ...
            ``,
            `The way Kiddo works: instead of cash that gets spent, the gift becomes a real investment in ${trimmedKidFirstName}'s name. The fund stays in your name (you're the legal custodian) until ${trimmedKidFirstName} is 18, then it's theirs.`,
            ``,
            `Set up ${trimmedKidFirstName}'s fund (about 5 minutes, no card needed) and ${trimmedGifterName}'s gift flows automatically.`,
            ``,
            claimUrl,
            ``,
