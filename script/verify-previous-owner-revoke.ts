...
// Uses the Dunphy demo: Haley owns her transferred fund, Phil is the
// previous custodian. RESTORES the demo state at the end (the founder's
// "Your part of the story" demo surface depends on Phil's window being open).
...
async function login(email: string): Promise<string> {
  const res = await fetch(`${baseUrl}/api/auth/login`, {
    ...
    body: JSON.stringify({ email, password: "dunphyfamily" }),
  });
...
async function main() {
  const haley = await login("haley@dunphyfamily.com");
  const phil = await login("phil@dunphyfamily.com");

  // Find Haley's transferred fund (she owns it; Phil is previousOwner).
  ...
  if (!fund) throw new Error(`no transferred fund found for Haley (${list.length} funds)`);
  const fundId = String(fund.id);
  console.log(`fund: ${fundId} (previousOwnerId ${String(fund.previousOwnerId).slice(0, 8)}...)\n`);

  // Baseline: Phil CAN see it.
  const before = await get(`/api/funds/${fundId}/dashboard-summary`, phil);
  before.status === 200 ? ok("baseline: Phil (previous owner) can read the fund") : bad(`baseline read: ${before.status}`);
  ...
  philListBefore.some((f) => String(f.id) === fundId) ? ok("baseline: fund appears in Phil's fund list") : bad("baseline: fund missing from Phil's list");

  // PROBE: Phil cannot revoke (he's view-only on this fund).
  const philTries = await post(`/api/funds/${fundId}/revoke-previous-owner-access`, phil);
  philTries.status === 403 ? ok("probe: Phil cannot call revoke (403 view-only)") : bad(`probe: Phil revoke returned ${philTries.status}`);

  // UI: the card renders for Haley in Settings, confirm flow works.
  const browser = await chromium.launch();
  const context = await browser.newContext({ viewport: { width: 1280, height: 900 } });
  await context.request.post(`${baseUrl}/api/auth/login`, { data: { email: "haley@dunphyfamily.com", password: "dunphyfamily" } });
  ...
  if (cardSeen) {
    ...
    r.status === 200 ? ok("API: Haley revokes successfully") : bad(`API revoke: ${r.status} ${JSON.stringify(r.json)}`);
  }
  await browser.close();

  // After: Phil is locked out everywhere.
  const after = await get(`/api/funds/${fundId}/dashboard-summary`, phil);
  after.status === 403 ? ok("after: Phil's read is now 403") : bad(`after read: ${after.status}`);
  ...
  !philListAfter.some((f) => String(f.id) === fundId) ? ok("after: fund gone from Phil's fund list") : bad("after: fund still in Phil's list");
  ...
  // RESTORE the demo: reopen Phil's window.
  ...
  restored.status === 200 ? ok("cleanup: demo restored (Phil's window reopened)") : bad(`cleanup: Phil still locked out: ${restored.status}`);
