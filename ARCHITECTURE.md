...
| Remaining JSON state files (custodian transfer outbox; **`.local/kid-view.json`** — Kid View enable/PIN-hash/hint/share-token, per-instance + ephemeral, so a multi-instance prod would lose Kid View setups + the demo seed writes it locally; anything else under `.local/*.json`) | Repeat the proven Postgres migration pattern (schema + migration + on-read backfill + reader swap). **Two files migrated; the rest are the same shape.** |
