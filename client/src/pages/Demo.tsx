...
import { ArrowRight, Users, Gift, ShieldCheck } from "lucide-react";
...
export default function Demo() {
  ...
  const handleLogin = async (email: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.message || "Demo login failed. Has the seed script been run?");
      }
      haptic("success");
      // Parents go to /dashboard; gifters go to /gifter (the gifter-side
      // dashboard where their lifetime gifts to the Dunphy kids show up).
      ...
      <main className="px-4 pb-20 pt-24 md:pb-28 md:pt-32">
        <div className="mx-auto max-w-5xl">
          ...
          <section className="mx-auto mt-12 grid max-w-4xl gap-4">
            <h2 className="font-heading text-lg font-semibold text-foreground">Start with Phil</h2>
          ...
          <section className="mx-auto mt-12 max-w-3xl rounded-2xl border border-border bg-card p-6 text-sm leading-relaxed text-muted-foreground">
