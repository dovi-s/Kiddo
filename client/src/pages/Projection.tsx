      ...
      <div className="kiddo-canvas px-4 py-6 space-y-6 max-w-lg md:max-w-4xl">
        ...
        <p className="text-[10.5px] text-muted-foreground/80 leading-relaxed text-center px-2">
          ...
          {isOwnerMode
            ...
            // individual account), so without this branch they'd hit the generic personal
            // line below — which assumes ongoing investing even at $0/mo. Fork on whether
            // they're modeling their own contributions; keep it owner-framed either way.
            ? (monthly > 0
                ? <>Assumes you keep investing {fmtMoney(monthly)}/mo at the chosen rate. It is your account now, so what you add next is your call.</>
                : <>Pure compound from today at the chosen rate. This fund is yours now, so what you add next is your call.</>)
            : isUtma
            ? (currentAge >= majorityAge
                // Non-owner past-majority edge (a UTMA not yet transferred): the
                // contributions-until-majority window has passed → pure compound,
                // present-tense.
