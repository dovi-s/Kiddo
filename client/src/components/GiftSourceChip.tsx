...
export function GiftSourceChip({ eventName, className = "" }: GiftSourceChipProps) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 max-w-full rounded-full bg-[hsl(var(--kiddo-cream-dark)/0.55)] px-2 py-0.5 text-3xs font-semibold text-foreground/75 ${className}`}
      title={name}
